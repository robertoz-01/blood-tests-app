class CheckEntry < ApplicationRecord
  ...
  def self.insert_user_entries(user_entries, blood_check)
    ...
    user_entries.each do |entry|
      if !existing_analyses_by_name.has_key?(entry.name)
        existing_analyses_by_name[entry.name] = Analysis.create(default_name: entry.name, unit: entry.unit, reference_lower: 0, reference_upper: 1)
      end
      if existing_entries_by_identifiers.has_key?(entry.identifier)
        ...
        CheckEntry.create(identifier: entry.identifier, value: entry.value, blood_check: blood_check, analysis: existing_analyses_by_name[entry.name])
      end
    end
  end
end
